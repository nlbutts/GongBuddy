...
INPUT_FILE=${ARM_BUILD_DIR}/fwimages/bootblock.bin
IMAGEGEN_TOOL=${X86_BUILD_DIR}/imagegen/imagegen

DEFAULT_OUTPUT_FILE=${ARM_BUILD_DIR}/fwimages/bootblock.image
...
if [[ -z "${2// }" ]]; then
    printf "\nMissing hardware compatibility flag, allowing imagegen to default\n"
    hw_comp=""
else
...
${IMAGEGEN_TOOL} -b ${INPUT_FILE} ${output_file} ${hw_comp} || exit 1
